...
- Added exported `nrd_pipeline()` for methodology-safe multi-year NRD workflows, processing each year independently through a user-supplied lazy pipeline before pooling per-year parquet outputs into one DuckDB-backed lazy table.
